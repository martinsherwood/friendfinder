...
<?php
	...
	//form function
	function renderForm($username = "", $email = "", $isadmin = "", $password = "", $error = "", $id = "") { ?>
                    ...
                    <?php if ($error != "") { echo "<div class=\"error\">" . $error . "</div>";} ?>
                            <form method="post" class="admin-form validate">
                                ...
                                <?php } ?>
                                    ...
                                    <label for="isadmin">Is Admin? (set 0 for false and 1 for true): *</label>
                                    <input type="text" name="isadmin" id="isadmin" value="<?php echo $isadmin; ?>" tabindex="3" maxlength="1" required>
                                    
                                    <label for="password">Password: *</label>
                                    <input type="text" name="password" id="password" placeholder="New password" value="" tabindex="4" autocomplete="off" required>
                                ...
                                if (isset($_POST["submit"]))
                                {
                                        ...
                                        if (is_numeric($_POST["id"]))
                                        {
                                                ...
                                                if ($username == "" || $password == "" || $email == "" || $isadmin == "") {
                                                        ...
                                                        renderForm($username = "", $email = "", $isadmin = "", $password = "", $error, $id);
                                                }
                                                else {
                                                        //update the record in the database
                                                        if ($stmt = $mysqli->prepare("UPDATE users SET username = ?, email = ?, isadmin = ?, password = ? WHERE id = ?")) {
                                                                $stmt->bind_param("ssisi", $username, $email, $isadmin, hash("sha512", $password), $id);
                                                                $stmt->execute();
                                                                $stmt->close();
                                                        }
                                                        //if query error
                                                        ...
                                                        //get information from database
                                                            ...
                                                            if (is_numeric($_GET["id"]) && $_GET["id"] > 0) {	
                                                                ...
                                                                if($stmt = $mysqli->prepare("SELECT * FROM users WHERE id = ?")) {
                                                                        ...
                                                                        $stmt->bind_result($id, $username, $email, $password, $date, $isadmin); //bind results into vars
                                                                        $stmt->fetch();
                                                                        
                                                                        renderForm($username, $email, $isadmin, $password, NULL, $id);
                                            ...
                                            /*New Admin - if the 'id' variable is not set in the URL*/
                                                    ...
                                                    //if the form's submit button is clicked, process the form
                                                    if (isset($_POST["submit"])) {
                                                            //get the form data
                                                            $username = htmlentities($_POST["username"], ENT_QUOTES);
                                                            $email = htmlentities($_POST["email"], ENT_QUOTES);
                                                            ...
                                                            if ($username == "" || $email == "" || $isadmin == "" || $password == "") {
                                                                    //if they are empty, show an error message and re-display the form
                                                                    $error = 'Error: Please fill in all required fields.';
                                                                    renderForm($username, $email, $isadmin, $password, $error);
                                                            }
                                                            else {
                                                                    //insert into the database
                                                                    if ($stmt = $mysqli->prepare("INSERT users (username, email, isadmin, password) VALUES (?, ?, ?, ?)")) {
                                                                            $stmt->bind_param("ssis", $username, $email, $isadmin, hash("sha512", $password));
                                                                            $stmt->execute();
                                                                            $stmt->close();
                                                                    }
